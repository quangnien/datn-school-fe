import { SET_ERRORS, STUDENT_UPW } from "../../../redux/actionTypes";
...
const Body = () => {
  ...
  useEffect(() => {
    if (store.errors || store.student.studentupwed) {
      setLoading(false);
      if (store.student.studentupwed) {
        setNewPassword("");
        setConfirmPassword("");
        setOldPassword("");
        setLoading(false);
        ...
        dispatch({ type: STUDENT_UPW, payload: false });
      }
    } else {
      setLoading(true);
    }
  }, [store.errors, store.student.studentupwed]);
